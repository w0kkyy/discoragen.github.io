  ...
  function openViewPostModal(targetMsgId) {
    ...
    postsToRender.forEach(msg => {
      ...
      const upVotes = msg.reactions ? (msg.reactions['👍'] || msg.reactions['❤'] || 0) : 0;
...
//  - У КАЖДОГО клиента (у всех открытых сайтов, не только у админа) работает live-подписка
//    (onSnapshot) на последние 50 уведомлений. Она сама выкидывает из списка (и из бейджа)
//    всё, что старше 24 часов, и заодно пытается удалить такие документы из базы —
//    это и есть "автоудаление через 24 часа" без отдельного сервера/Cloud Function.
//  - "Прочитано/не прочитано" считается локально в этом браузере (localStorage), отдельно
//    для каждого uid — поэтому бейдж корректно меняется при смене Telegram-аккаунтов.
const NOTIF_TTL_MS = 24 * 60 * 60 * 1000; // 24 часа — срок жизни уведомления
let notifListenerUnsub = null;
let latestNotifications = [];

// Ключ в localStorage для отметки "последнее прочитанное время" конкретного пользователя
function notifSeenKey(uid) {
  return 'notif_seen_' + (uid || 'guest');
}
function getNotifSeenAt(uid) {
  return parseInt(localStorage.getItem(notifSeenKey(uid)) || '0', 10);
}
function setNotifSeenAt(uid, ts) {
  try { localStorage.setItem(notifSeenKey(uid), String(ts)); } catch (e) {}
}
    ...
    .onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        const ts = (data.createdAt && typeof data.createdAt.toDate === 'function')
          ...
          : now; // пока serverTimestamp не подтвердился локально — считаем "только что"
        if (now - ts > NOTIF_TTL_MS) {
          // Уведомлению больше 24 часов — подчищаем за собой. Ошибки (например,
          // если правила безопасности запретят удаление гостю) молча игнорируем.
          doc.ref.delete().catch(() => {});
        } else {
          fresh.push({ id: doc.id, text: data.text || '', authorName: data.authorName || '', ts: ts });
        }
      });
      latestNotifications = fresh;
      renderNotifications();
    }, (err) => {
...
function renderNotifications() {
  ...
  const seenAt = getNotifSeenAt(user ? user.uid : null);
  const unreadCount = latestNotifications.filter((n) => n.ts > seenAt).length;
...
// Открыть/закрыть дропдаун уведомлений; при открытии отмечаем всё как прочитанное
window.toggleNotifDropdown = function () {
  ...
  if (willOpen) {
    const user = (typeof auth !== 'undefined' && auth) ? auth.currentUser : null;
    setNotifSeenAt(user ? user.uid : null, Date.now());
    setTimeout(renderNotifications, 200); // прячем бейдж чуть погодя, без резкого мигания
  }
};
...
window.openNotifyAdminModal = function () {
  ...
  const input = document.getElementById('broadcastNotifInput');
...
window.sendBroadcastNotification = function () {
  ...
  if (!text) { showToast('Введите текст уведомления'); return; }
