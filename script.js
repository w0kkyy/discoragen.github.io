  ...
  function toggleFullScreen() {
    if (!document.fullscreenElement) {
      document.documentElement.requestFullscreen().catch(err => {
        showToast('Ошибка включения полноэкранного режима', 'error');
      });
  ...
  function renderReelCard(index) {
    ...
    if (isAlbum) {
      ...
      mediaHTML = `
        <div class="${isNsfwHidden ? 'nsfw-blurred' : ''}" data-nsfw-media="${msg.id}" style="position: relative;">
          <div id="albumFrame_${msg.id}">
            ${firstIsVideo
              ? `<video controls src="${firstSrc}" style="max-height: 520px; width: 100%; object-fit: contain;"></video>`
              : `<img src="${firstSrc}" style="max-height: 520px; width: 100%; object-fit: contain;" alt="Media" />`}
          </div>
          <div class="album-nav-arrow prev" onclick="changeAlbumSlide('${msg.id}', -1, event)"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></div>
          <div class="album-nav-arrow next" onclick="changeAlbumSlide('${msg.id}', 1, event)"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></div>
          <div class="album-dots-row">${dotsHTML}</div>
          <div class="album-hint-badge"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>Это альбом, можно листать</div>
        </div>
      ...
      if (msg.isExternalLink) {
        ...
        mediaHTML = `<div class="${isNsfwHidden ? 'nsfw-blurred' : ''}" data-nsfw-media="${msg.id}"><video controls src="${msg.image}" style="max-height: 520px; width: 100%; object-fit: contain;"></video></div>${isNsfwHidden ? nsfwOverlayHTML(msg.id, false) : ''}`;
        ...
        mediaHTML = `<div class="${isNsfwHidden ? 'nsfw-blurred' : ''}" data-nsfw-media="${msg.id}"><img src="${msg.image}" style="max-height: 520px; width: 100%; object-fit: contain;" alt="Media" /></div>${isNsfwHidden ? nsfwOverlayHTML(msg.id, false) : ''}`;
      }
    }
  ...
  function handleLocalAvatarFile(event) {
    ...
    if (isGif && !isPremiumActive) {
      showToast('GIF-аватарки доступны только по Premium-подписке', 'error');
      return;
    }
  ...
  */
  ...
  function nsfwOverlayHTML(msgId, compact) {
    if (compact) {
      return `
        <div class="nsfw-censor-badge" data-nsfw-overlay="${msgId}" onclick="revealNsfwPost('${msgId}', event)" title="Контент скрыт цензурой">
  ...
  function renderAlbumFrame(msgId, imgs, idx) {
    ...
    frame.innerHTML = isVideo
      ? `<video controls src="${src}" style="max-height: 520px; width: 100%; object-fit: contain;"></video>`
      : `<img src="${src}" style="max-height: 520px; width: 100%; object-fit: contain;" alt="Media" />`;
    frame.querySelectorAll('.album-dot').forEach((dot, i) => dot.classList.toggle('active', i === idx));
  }
  ...
  function renderPhotosGrid() {
    ...
    feedList.forEach(p => {
      ...
      if (p.isExternalLink) {
        ...
        previewContent = `<div class="photo-media-wrap${blurClass}" data-nsfw-media="${p.id}"><img src="${p.image}" loading="lazy" alt=""></div>${nsfwOverlay}`;
      }
