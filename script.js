  ...
  function switchTab(tabName) {
    ...
    const mbnMap = { photos: 'mbnPhotos', videos: 'mbnVideos', newpost: 'mbnNewPost' };
  ...
  // Дублирующие иконки темы внутри мобильного бургер-меню (кнопка темы
  // на мобильных перенесена туда из шапки, см. mobThemeToggleBtn ниже)
  const mobSunIcon = document.getElementById('mobSunIcon');
  const mobMoonIcon = document.getElementById('mobMoonIcon');
  ...
  const mobThemeToggleBtn = document.getElementById('mobThemeToggleBtn');
  if (mobThemeToggleBtn) {
    mobThemeToggleBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      setTheme(!document.body.classList.contains('light-theme'));
    });
  }
