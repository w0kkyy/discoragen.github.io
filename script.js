  ...
  function renderReelCard(index) {
    ...
    const isSubscribed = !!(currentUserProfile && currentUserProfile.friends && currentUserProfile.friends.includes(msg.userId || msg.author));
  ...
  function openUserProfile(userId, fallbackName, fallbackAvatar) {
    ...
    const friendsCountEl = document.getElementById('modalUserFriendsCount');
    if (isMe) {
      friendsCountEl.textContent = (currentUserProfile && currentUserProfile.friends) ? currentUserProfile.friends.length : 0;
    } else if (userId && !userId.startsWith('guest_')) {
      db.collection('users').doc(userId).get().then(doc => {
        friendsCountEl.textContent = (doc.exists && doc.data().friends) ? doc.data().friends.length : 0;
  ...
  */
  ...
  window.toggleAuthorSubscription = function(targetId, authorName, msgId) {
    ...
    if (!currentUserProfile.friends) currentUserProfile.friends = [];
    const idx = currentUserProfile.friends.indexOf(targetId);
    const badge = document.getElementById(`subBadge_${msgId}`);
    let nowSubscribed;
    if (idx > -1) {
      currentUserProfile.friends.splice(idx, 1);
      nowSubscribed = false;
      showToast(`Вы отписались от @${authorName}`);
    } else {
      currentUserProfile.friends.push(targetId);
      nowSubscribed = true;
      showToast(`Вы подписались на @${authorName}`);
    }
    if (badge) {
      badge.classList.toggle('subscribed', nowSubscribed);
      badge.title = nowSubscribed ? 'Отписаться' : 'Подписаться';
      badge.innerHTML = nowSubscribed
        ? `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>`
        : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>`;
    }
    db.collection('users').doc(user.uid).update({
